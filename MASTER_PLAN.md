...
**Статус:** ✅ Закрыт 2026-04-01 — мёртвые стабы удалены; реальный сервис в `services/server/access_key_svc.rs`
...
**Статус:** ✅ Закрыт 2026-04-01 — `AuthUser` extractor добавлен в `run_playbook` handler; `user_id` передаётся явно
...
**Статус:** ✅ Закрыт 2026-04-01 — DingTalk HMAC-SHA256+timestamp реализован; generic webhook уже имел HMAC
...
**Статус:** ✅ Закрыт 2026-04-01 — `JobPool::shutdown()` с `AtomicBool` + 30s timeout; `cmd_server.rs` вызывает после сигнала
...
**Статус:** ✅ Закрыт 2026-04-01 — все оставшиеся `.unwrap()` в `#[cfg(test)]` блоках; production код чист
...
**Статус:** ✅ Закрыт 2026-04-01 — `src/logging.rs`: `tracing_subscriber::fmt().json()` по `VELUM_LOG_FORMAT=json`
...
**Статус:** ✅ Закрыт 2026-04-01 — `/healthz`, `/readyz`, `/api/health/live`, `/api/health/ready`, `/api/health/full` зарегистрированы
...
|---|---|---|---|---|
| P-01 | access_key_installation_service → store | 🔴 | M | ✅ 2026-04-01 |
| P-02 | playbook_run_service user_id из auth | 🟠 | S | ✅ 2026-04-01 |
| P-03 | HMAC подпись webhook | 🟠 | S | ✅ 2026-04-01 |
| P-04 | Graceful shutdown SIGTERM | 🟠 | M | ✅ 2026-04-01 |
| P-05 | Убрать unwrap() из API handlers | 🟡 | M | ✅ 2026-04-01 |
| P-06 | Structured JSON logging | 🟡 | S | ✅ 2026-04-01 |
| P-07 | /healthz + /readyz endpoints | 🟡 | S | ✅ 2026-04-01 |
| P-08 | Тесты критических путей | 🟡 | M | ✅ 2026-04-01 |
...
## 21. Единый план завершения проекта (v1.0 Release)

> Составлен 2026-04-01. Три конечные цели, 21 задача, 5 спринтов.

### Конечные цели

| ID | Цель | Готовность |
|---|---|---|
| G-1 | Feature parity с Go-оригиналом (velum/velum) | ~97% |
| G-2 | Kubernetes UI — полная функциональность | ~92% |
| G-3 | Production-ready: hardening, CI/CD, документация, релиз v1.0 | ~20% |

---

### Открытые задачи

#### Блок P — Production Hardening

| ID | Задача | Приоритет | Сложность | Статус |
|---|---|---|---|---|
| P-01 | access_key_installation_service → реальный store | 🔴 | M | ✅ 2026-04-01 |
| P-02 | playbook_run_service: user_id из AuthUser | 🟠 | S | ✅ 2026-04-01 |
| P-03 | HMAC-SHA256 подпись webhook payload | 🟠 | S | ✅ 2026-04-01 |
| P-04 | Graceful shutdown при SIGTERM (ждать running jobs) | 🟠 | M | ✅ 2026-04-01 |
| P-05 | Убрать .unwrap() из API handlers (62 места) | 🟡 | M | ✅ 2026-04-01 |
| P-06 | Structured JSON logging via tracing-subscriber | 🟡 | S | ✅ 2026-04-01 |
| P-07 | /healthz + /readyz эндпоинты (K8s probes) | 🟡 | S | ✅ 2026-04-01 |
| P-08 | Тесты критических путей (4 новых теста) | 🟡 | M | ✅ 2026-04-01 |
| P-09 | OpenAPI spec: K8s WS endpoints + актуализация | 🟡 | S | ⬜ |
| P-10 | cargo-tarpaulin coverage в CI pipeline | 🟢 | S | ⬜ |
| P-11 | CHANGELOG.md по Keep a Changelog | 🟢 | S | ⬜ |

#### Блок K — Kubernetes UI (оставшиеся пункты)

| ID | Задача | Приоритет | Сложность | Статус |
|---|---|---|---|---|
| K-01 | Cluster-wide Events WebSocket stream | 🟠 | M | ✅ 2026-04-01 |
| K-02 | YAML editor: inline dry-run tab | 🟡 | M | ✅ 2026-04-01 |
| K-03 | Kubeconfig encryption at rest (AES-256) | 🟠 | S | ✅ 2026-04-01 |
| K-04 | API performance benchmarks (p50/p95) | 🟡 | S | ⬜ |
| K-05 | Mobile K8s UI + WCAG 2.1 AA | 🟢 | L | ⬜ |

#### Блок D — Documentation & Release

| ID | Задача | Приоритет | Сложность | Статус |
|---|---|---|---|---|
| D-01 | E2E deployment testing (Docker + K8s) | 🟠 | M | ⬜ |
| D-02 | Postman collection verification (75+ endpoints) | 🟡 | S | ⬜ |
| D-03 | README.md — полное обновление | 🟡 | S | ⬜ |
| D-04 | DEPLOY.md — production deployment guide | 🟡 | M | ⬜ |
| D-05 | GitHub Release v1.0.0 (binaries, Docker image) | 🟢 | S | ⬜ |

---

### 5-спринтовый план выполнения

#### Спринт 1 — Production Safety (приоритет: безопасность и надёжность)
**Цель:** устранить критические риски до деплоя в production

| # | Задача | Файл |
|---|---|---|
| 1 | P-01: AccessKeyServiceImpl → store | `services/access_key_installation_service.rs` |
| 2 | P-04: Graceful shutdown SIGTERM | `cli/cmd_server.rs` |
| 3 | P-02: playbook_run user_id | `services/playbook_run_service.rs` |
| 4 | P-03: HMAC webhook signature | `services/alert.rs` |
| 5 | K-03: Kubeconfig encryption | `api/handlers/kubernetes/client.rs` + `utils/encryption.rs` |

#### Спринт 2 — Quality & Observability
**Цель:** надёжность, мониторинг, устойчивость к сбоям

| # | Задача | Файл |
|---|---|---|
| 1 | P-05: убрать .unwrap() | `api/handlers/**/*.rs` (62 места) |
| 2 | P-06: JSON logging | `cli/cmd_server.rs`, `Cargo.toml` |
| 3 | P-07: /healthz + /readyz | `api/routes.rs` |
| 4 | P-08: тесты критических путей | `tests/` |
| 5 | K-01: Events WebSocket stream | `api/handlers/kubernetes/events_ws.rs` |

#### Спринт 3 — K8s UI Polish
**Цель:** завершить недостающие K8s-фичи

| # | Задача | Файл |
|---|---|---|
| 1 | K-02: YAML dry-run inline tab | `web/public/k8s-*.html` |
| 2 | K-04: API benchmarks | `benches/` |
| 3 | P-09: OpenAPI актуализация | `api-docs.yml` |

#### Спринт 4 — E2E Testing & Documentation
**Цель:** проверить всё работает в реальном окружении

| # | Задача | Файл |
|---|---|---|
| 1 | D-01: E2E deployment testing | Docker + K8s manifests |
| 2 | D-02: Postman verification | `.postman/` |
| 3 | D-03: README.md | `README.md` |
| 4 | D-04: DEPLOY.md | `DEPLOY.md` |

#### Спринт 5 — Release
**Цель:** опубликовать v1.0.0

| # | Задача | Файл |
|---|---|---|
| 1 | P-10: coverage в CI | `.github/workflows/rust.yml` |
| 2 | P-11: CHANGELOG.md | `CHANGELOG.md` |
| 3 | D-05: GitHub Release v1.0.0 | GitHub Releases |
| 4 | K-05: Mobile + WCAG (опционально) | `web/public/k8s-*.html` |

---

### Критерии "Проект завершён"

- [ ] `cargo test` — 100% green (все тесты)
- [ ] `cargo clippy -- -D warnings` — 0 warnings
- [ ] P-01..P-04 — реализованы и задокументированы
- [ ] K-03 — kubeconfig зашифрован в БД
- [ ] `/healthz` + `/readyz` — отвечают корректно
- [ ] E2E тест деплоя пройден (Docker + K8s)
- [ ] README.md + DEPLOY.md обновлены
- [ ] CHANGELOG.md заполнен
- [ ] GitHub Release v1.0.0 опубликован с binaries
