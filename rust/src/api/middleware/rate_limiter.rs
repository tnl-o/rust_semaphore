    ...
    async fn maybe_cleanup(&self) {
        ...
        if now.duration_since(*last_cleanup_guard) > self.cleanup_interval {
            ...
            clients.retain(|_, info| {
                now.duration_since(info.period_start) < period
            });
...
pub async fn app_api_rate_limit(
    ...
    if !state.rate_limiter_api.is_allowed(&ip).await {
        ...
        headers.insert("X-RateLimit-Limit", state.rate_limiter_api.config.max_requests.to_string().parse().unwrap());
        headers.insert("X-RateLimit-Remaining", remaining.to_string().parse().unwrap());
        headers.insert("Retry-After", state.rate_limiter_api.config.period_secs.to_string().parse().unwrap());
...
pub async fn app_auth_rate_limit(
    ...
    if !state.rate_limiter_auth.is_allowed(&ip).await {
        ...
        let headers = response.headers_mut();
        headers.insert("X-RateLimit-Limit", state.rate_limiter_auth.config.max_requests.to_string().parse().unwrap());
        headers.insert("X-RateLimit-Remaining", remaining.to_string().parse().unwrap());
        headers.insert("Retry-After", state.rate_limiter_auth.config.period_secs.to_string().parse().unwrap());
...
mod tests {
    ...
    async fn test_rate_limiter_auth_strict() {
        ...
        for i in 0..5 {
            assert!(limiter.is_allowed("auth_user").await, "Auth request {} should be allowed", i);
        }
        
        assert!(!limiter.is_allowed("auth_user").await, "6th auth request should be blocked");
    }
    ...
    async fn test_rate_limiter_sensitive() {
        ...
        for i in 0..10 {
            assert!(limiter.is_allowed("sensitive_op").await, "Sensitive request {} should be allowed", i);
        }
        
        assert!(!limiter.is_allowed("sensitive_op").await, "11th sensitive request should be blocked");
    }
}
