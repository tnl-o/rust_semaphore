...
use crate::db::store::{Store, WorkflowManager, TemplateManager, TaskManager, InventoryManager, RepositoryManager, EnvironmentManager};
use crate::models::workflow::{Workflow, WorkflowNode, WorkflowEdge, WorkflowRun};
...
impl WorkflowExecutionContext {
    pub fn new(workflow: Workflow, nodes: Vec<WorkflowNode>, edges: Vec<WorkflowEdge>, run: WorkflowRun) -> Self {
        ...
        for edge in &self.edges {
            if edge.from_node_id == node_id {
                let should_execute = match edge.condition.as_str() {
                    ...
                    "failure" => matches!(task_status, TaskStatus::Error | TaskStatus::Stopped | TaskStatus::NotExecuted),
                    "always" => true,
                    _ => false,
                };
    ...
    pub fn is_complete(&self) -> bool {
        self.node_statuses.values().all(|s| {
            matches!(s, NodeExecutionStatus::Success(_) | NodeExecutionStatus::Failed(_) | NodeExecutionStatus::Skipped)
        })
    }
    
    /// Проверить, есть ли запущенные узлы
    pub fn has_running_nodes(&self) -> bool {
        self.node_statuses.values().any(|s| matches!(s, NodeExecutionStatus::Running(_)))
    }
}
...
impl WorkflowExecutor {
    pub fn new(state: Arc<AppState>, workflow: Workflow, nodes: Vec<WorkflowNode>, edges: Vec<WorkflowEdge>, run: WorkflowRun) -> Self {
    ...
    pub async fn execute(&self) -> Result<()> {
        ...
        self.state.store.update_workflow_run_status(
        ...
        while !queue.is_empty() {
            ...
            while let Some(node_id) = queue.pop() {
                ...
                let handle = tokio::spawn(async move {
                    executor.execute_node_sync(node_id).await
                });
    ...
    async fn execute_node_sync(&self, node_id: i32) -> Result<Vec<i32>> {
        ...
        let (template, task, project_id) = {
            ...
            let status = ctx.node_statuses.get(&node_id).cloned().unwrap_or(NodeExecutionStatus::Pending);
            if !matches!(status, NodeExecutionStatus::Pending) {
                println!("[workflow_executor] Node {} already executed: {:?}", node_id, status);
                return Ok(Vec::new());
            }
            ...
            let template = self.state.store.get_template(ctx.project_id, node.template_id).await
                .map_err(|e| Error::NotFound(format!("Template {} not found: {}", node.template_id, e)))?;
            ...
            ctx.node_statuses.insert(node_id, NodeExecutionStatus::Running(created_task.clone()));
    ...
    async fn run_task(&self, task: Task, template: Template, project_id: i32) -> Result<TaskStatus> {
        ...
        let inventory = if let Some(inv_id) = task.inventory_id.or(template.inventory_id) {
            self.state.store.get_inventory(project_id, inv_id).await.unwrap_or_default()
        } else {
            Inventory::default()
        };

        let repository = if let Some(repo_id) = task.repository_id.or(template.repository_id) {
            self.state.store.get_repository(project_id, repo_id).await.unwrap_or_default()
        } else {
            Repository::default()
        };

        let environment = if let Some(env_id) = task.environment_id.or(template.environment_id) {
            self.state.store.get_environment(project_id, env_id).await.unwrap_or_default()
        } else {
            Environment::default()
        };
    ...
    async fn finalize_workflow(&self) -> Result<()> {
        ...
        let has_failures = ctx.node_statuses.values().any(|s| matches!(s, NodeExecutionStatus::Failed(_)));
        
        let (final_status, message) = if has_failures {
            ("failed".to_string(), Some("Workflow completed with failures".to_string()))
        } else {
            ("success".to_string(), Some("Workflow completed successfully".to_string()))
        };
...
pub async fn run_workflow(
    ...
    let run = state.store.create_workflow_run(workflow_id, project_id).await?;
    ...
    tokio::spawn(async move {
        if let Err(e) = executor.execute().await {
            ...
            let _ = state.store.update_workflow_run_status(
                run.id,
                "failed",
