...
impl HelmClient {
    ...
    pub fn install(
        ...
        info!(
            "Installing Helm chart '{}' as release '{}' in namespace '{}'",
            chart.name, release_name, ns
        );

        let mut cmd = Command::new(&self.helm_path);
        cmd.arg("install").arg(release_name).arg("-n").arg(ns);
    ...
    pub fn upgrade(
        ...
        info!(
            "Upgrading Helm release '{}' in namespace '{}'",
            release_name, ns
        );

        let mut cmd = Command::new(&self.helm_path);
        cmd.arg("upgrade").arg(release_name).arg("-n").arg(ns);
    ...
    pub fn list_releases(&self, namespace: Option<&str>) -> Result<Vec<HelmRelease>> {
        ...
        let mut cmd = Command::new(&self.helm_path);
        cmd.arg("list").arg("-n").arg(ns).arg("-o").arg("json");
    ...
    pub fn uninstall(&self, release_name: &str, namespace: Option<&str>) -> Result<()> {
        ...
        let mut cmd = Command::new(&self.helm_path);
        cmd.arg("uninstall").arg(release_name).arg("-n").arg(ns);
    ...
    pub fn rollback(
        ...
        info!(
            "Rolling back release '{}' to revision {} in namespace '{}'",
