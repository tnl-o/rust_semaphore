...
impl SqlDb {
    ...
    pub async fn get_schedules(&self, project_id: i32) -> Result<Vec<Schedule>> {
        let rows = sqlx::query(
            "SELECT * FROM schedule WHERE project_id = $1 ORDER BY name"
        )
        .bind(project_id)
        ...
        Ok(rows.into_iter().map(|row| Schedule {
            ...
            delete_after_run: row.try_get::<bool, _>("delete_after_run").ok().unwrap_or(false),
        }).collect())
    }
    ...
    pub async fn get_schedule(&self, project_id: i32, schedule_id: i32) -> Result<Schedule> {
        let row = sqlx::query(
            "SELECT * FROM schedule WHERE id = $1 AND project_id = $2"
        )
        .bind(schedule_id)
        .bind(project_id)
