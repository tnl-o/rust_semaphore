...
impl SqlDb {
    ...
    pub async fn get_runner_by_token(&self, token: &str) -> Result<Runner> {
        let runner = sqlx::query_as::<_, Runner>("SELECT * FROM runner WHERE token = $1")
            .bind(token)
    ...
    pub async fn delete_global_runner(&self, runner_id: i32) -> Result<()> {
        let result = sqlx::query("DELETE FROM runner WHERE id = $1 AND project_id IS NULL")
            .bind(runner_id)
    ...
    pub async fn clear_runner_cache(&self, runner: &Runner) -> Result<()> {
        ...
        if let Some(project_id) = runner.project_id {
            sqlx::query(
                "UPDATE runner SET cleaning_requested = $1 WHERE id = $2 AND project_id = $3",
            )
            .bind(Utc::now())
            .bind(runner.id)
            .bind(project_id)
