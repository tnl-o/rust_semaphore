...
./start.sh native --init   # первый запуск
./start.sh native          # последующие запуски
```
...
./start.sh native --build  # Пересобрать backend
```
...
./start.sh hybrid --build  # Пересобрать backend
```
...
./start.sh docker --build  # Пересобрать образы
```
