...
- ✅ Запуск сервера — успешно (SQLite, порт 3000)
- ✅ Health check — ✅ БД в порядке
- ✅ Создание пользователя (CLI) — успешно
- ✅ Исправлен `cli/cmd_user.rs` — убрано двойное хэширование пароля
- ✅ Исправлен `db/sql/mod.rs` — добавлено хэширование в create_user
- ⚠️ Аутентификация — найдена проблема: POST /api/auth/login возвращает 401 с Content-Length: 0
- 🔍 Отладка: проблема в обработке ответа (StatusCode, Json(...)) в Axum
- ⏳ Ожидает: решение проблемы с POST ответом
