...
- ⚠️ Аутентификация — POST /api/auth/login работает, но 401 возвращает пустое тело
  - Выявлено: (StatusCode, Json(...)) не работает для ошибок в Axum
  - Временное решение: использовать Response напрямую
- ✅ Отправлено в origin/main (2 коммита)
- ✅ Синхронизация semaphore/rust с rust_semaphore завершена
  - Скопированы файлы: inventory.rs, cmd_user.rs, mod.rs, routes.rs, auth.rs
  - semaphore/rust собирается без ошибок
